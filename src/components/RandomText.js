...
const RandomText = ({ text, color, bgColor }) => {
    ...
    return (
        <div className="modern-react-captcha__captchaText" style={{ width: 'fit-content', padding: '3px 10px', backgroundColor: `${(bgColor === 'random' || !bgColor) ? generateRandomHexColor() : bgColor}` }}>
            {
                captcha.split('').map(unit => (
                    <span key={Math.random() - Math.random()} className='modern-react-captcha__character' style={{ color: `${color === 'random' ? generateRandomHexColor() : color}`, fontSize: `${fontSizes[Math.floor(Math.random() * fontSizes.length)]}px`, fontWeight: `${fontWeights[Math.floor(Math.random() * fontWeights.length)]}`, fontStyle: `${Math.random() > 0.5 ? 'italic' : 'normal'}`, fontFamily: `${fontFamilies[Math.floor(Math.random() * fontFamilies.length)]}` }}>{unit}</span>
                ))
            }
        </div>
    );
